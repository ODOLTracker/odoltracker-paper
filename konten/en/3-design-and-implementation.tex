...
\begin{itemize}[nolistsep]
  \item The User class contains data of application users, both admin and operators. This class has a one-to-many relationship with the VehicleDetection and Notification classes
  \item The TollGate class contains data of existing toll gates. This class has a one-to-many relationship with the VehicleDetection class
  \item The Notification class contains notification data in the application. This includes notifications sent by the system and notifications sent by admin to operators.
  \item The VehicleDetection class contains vehicle detection data in the application. In this class, the status of vehicles is verified, whether the vehicle is overdimension or not. This class has a one-to-many relationship with the Image class
  \item The Image class contains image data of detected overdimension vehicles. Images are not stored in the database, but only store links to images in Cloudinary cloud storage.
\end{itemize}
